module Ship
  module Ext::Item
    ...
    included do
      ...
      belongs_to :box_proxy_buy, ->(o) { where(box_specification_id: o.good.box_specification_id, price: o.single_price) }, foreign_key: :organ_id, primary_key: :organ_id, optional: true

      after_save_commit :init_box_hold, if: -> { good.respond_to?(:box_specification_id) && saved_change_to_status? && ['paid'].include?(status) }
      after_save_commit :init_box_proxy_buy, if: -> { good.respond_to?(:box_specification_id) && saved_change_to_status? && ['paid'].include?(status) }
    end
