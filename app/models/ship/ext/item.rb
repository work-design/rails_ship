module Ship
  module Ext::Item
    ...
    included do
      ...
      after_save_commit :init_box_proxy_buy, if: -> { good_type == 'Ship::BoxHost' && saved_change_to_status? && ['deliverable'].include?(status) }
      after_save_commit :reset_proxy_count, if: -> { saved_change_to_rest_number? }
      after_destroy_commit :reset_proxy_count
    end
